...
def scale_dots(gi_min: float, current: float) -> float:
    return 240 * (1 + ((current - gi_min) / (gi_min)))
...
def print_dataclass(dc: dataclass,print_only: List = None):
    if print_only is not None:
        for field in set(dc.__dataclass_fields__).intersection(set(print_only)):
...
def set_axes(color_name = "black",disable_ticks=True):
